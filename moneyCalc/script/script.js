const generateId = () => `text${Math.round(Math.random()*1e8).toString(16)}`;
const
...
let dbOperation = JSON.parse(localStorage.getItem('calculation')) || [];
...
const init = () => {
    ...
    updateBalance();
    localStorage.setItem('calculation', JSON.stringify(dbOperation));
}
init();
